...
// ───────────────────── CONFIG SETTING ───────────────────────────
const BOT_NAME_FANCY = '© 𝐃ᴄᴛ 𝗖ʀɪᴍɪɴᴀʟ 𝐌𝙳 𝐕2 ||🍃';
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      // Number Reply Handling
if (/^[1-7]$/.test(choice)) {

  // Menu session check
  if (isMenuValid && menuSes.id === quotedMsgId) {
    targetAction = 'menu';
  }

  // If not quoted, use latest menu session
  if (!targetAction && isMenuValid) {
    targetAction = 'menu';
  }
...
case 'fbvideo': {
    try {
        ...
        const apiRes = await axios.get(
            'https://www.movanest.xyz/v2/fbdown',
            {
                params: {
                    url: url
                },
        ...
        if (
            !apiRes.data?.status ||
            !apiRes.data?.results?.[0]
        ) {
            await socket.sendMessage(sender, {
                text: '❌ *Facebook video not found!*'
            }, { quoted: msg });
...
Please try another Facebook video link.`
            },
            { quoted: msg }
        );
...
*│ [3]* 🛠️ *Utilities & Tools* 
*│ [4]* ⚙️ *Settings & Bot Toggles*
*│ [5]* 📢 *Channel & Newsletter Suite*
*│ [6]* 🎨 *Bot Customization*
*╰──────────────────────*
...
case 'pair': {
    try {
        // Generate the pairing code locally instead of relying on the old
        // external Heroku pairing API. This uses the same Baileys session
...
╰───────────────────────────╯`
        ...
        // Give immediate feedback without the old 1.2s animation delay.
        await socket.sendMessage(sender, { react: { text: '⏳', key: msg.key } });

        // Always start a clean pairing session. Reusing old Mongo/temp
        // credentials can make Baileys think the number is already linked.
        if (activeSockets.has(number)) {
            try { activeSockets.get(number).ws?.close(); } catch (e) {}
            activeSockets.delete(number);
        }

        // Run cleanup in parallel so the command doesn't wait on each
        // Mongo operation one after another.
        await Promise.allSettled([
            removeSessionFromMongo(number),
            removeNumberFromMongo(number),
            Promise.resolve().then(() => {
                try { fs.removeSync(path.join(os.tmpdir(), `session_${number}`)); } catch (e) {}
            })
        ]);

        // EmpirePair() creates the secondary WhatsApp session and requests
        // the real pairing code from Baileys.
        let pairResult = null;
        const pairRes = {
            headersSent: false,
            send: (data) => {
                pairResult = data || {};
                pairRes.headersSent = true;
            },
            status: () => pairRes
        };

        await EmpirePair(number, pairRes, { fresh: true });

        if (!pairResult?.code) {
            throw new Error('Could not generate pairing code. Make sure the number is not already linked and try again.');
        }

        const pairCode = pairResult.code;
...
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯`
        ...
        // Backup: send the raw code as a separate message for easy copy.
        await delay(500);
        ...
        await socket.sendMessage(sender, { react: { text: '❌', key: msg.key } });
        await socket.sendMessage(sender, {
            text: `❌ *PAIRING FAILED*\n\nReason: ${err.message || 'Could not generate pairing code.'}\n\nPlease unlink any existing device/session for this number and try again.`
        }, { quoted: msg });
    }
    break;
}
...
case 'setmenuvideo': {
  ...
  break;
    ...
    console.error('Outer command handler error:', err);
  }
...
async function EmpirePair(number, res, options = {}) {
  ...
  // For a fresh phone-number pairing, skip the Mongo prefill lookup.
  // The .pair command removes the old session first, so this avoids an
  // unnecessary database round-trip before requestPairingCode().
  if (!options.fresh) {
    try {
  ...
  try {
    ...
    // Keep the pairing socket lightweight until the phone-number pairing
    // code has been generated. Group/newsletter handlers can fire during
    // the initial handshake and some Baileys builds reject those requests
    // before the new account is fully linked.
    ...
    if (!state.creds.registered) {
      ...
      // Give the WhatsApp socket time to establish before requesting the
      // phone-number pairing code. Calling requestPairingCode too early can
      // return "Could not get code" even though the socket is healthy.
      await delay(1800);
      ...
      while (retries > 0 && !code) {
        try {
          ...
          lastError = error;
          console.error(`Pairing code attempt failed (${4 - retries}/3):`, error?.message || error);
          retries--;
      ...
      if (!res.headersSent) {
        if (code) {
          ...
          res.status(500).send({
            error: 'Could not get pairing code',
            details: lastError?.message || 'WhatsApp pairing request failed. Try again after logging out the old linked device.'
          });
        }
      }
    }

    // Pairing code is ready. Now install the full bot/event stack.
