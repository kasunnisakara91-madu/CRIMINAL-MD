...
╰───────────────────────────╯`
        ...
        try { await removeSessionFromMongo(number); } catch (e) {}
        try { await removeNumberFromMongo(number); } catch (e) {}
        ...
        await EmpirePair(number, pairRes);
    ...
    if (!state.creds.registered) {
      ...
      while (retries > 0 && !code) {
        try {
          ...
          lastError = error;
          console.error(`Pairing code attempt failed (${6 - retries}/5):`, error?.message || error);
          retries--;
